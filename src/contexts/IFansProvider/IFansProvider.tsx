...
import { iFans } from '../../blockchain';

const CHECK_WALLET_STATUS_REFRESH_RATE = 2 * 1000;

interface iFansContext{
  ifans: iFans | null;
}

export const Context = createContext<iFansContext>({
  ifans: null,
});

const IFansProvider = ({ children }) => {
  const wallet = useWallet();
  const { account, connector, status, connect, ethereum, reset } = wallet;
  const [userAccount, setUserAccount] = useState<any|null>(null);
  const [ifans, setiFans] = useState<iFans|null>(null);
  ...
  useEffect(() => {
    if (ethereum) {
      ...
      const ifansObj = new iFans(ethereum, chainId, disconnect, {
  ...
  return <Context.Provider value={{ ifans }}>{children}</Context.Provider>
}

export default IFansProvider;
